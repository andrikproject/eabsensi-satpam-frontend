...
async function loadDB() {
  try {
    const raw = await loadFromGithubRaw();
    db = normalizeDB(raw);
  ...
  const cached = localStorage.getItem("satpam-db-cache");
  if (cached) {
    try {
      db = normalizeDB(JSON.parse(cached));
      toast("Mode cache lokal: cek config GitHub bila data belum sinkron.");
      return;
    } catch (_) {}
  }

  db = normalizeDB(DEFAULT_DB);
  toast("Mode demo: isi config.js agar data tersimpan ke GitHub.");
}

function hasGithubConfig() {
  return Boolean(CONFIG.GITHUB_OWNER && !CONFIG.GITHUB_OWNER.includes("ISI_") && CONFIG.BACKEND_REPO && CONFIG.DATA_PATH);
}

async function loadFromGithubRaw() {
  if (!hasGithubConfig()) throw new Error("Config GitHub belum diisi");
  const url = `https://raw.githubusercontent.com/${encodeURIComponent(CONFIG.GITHUB_OWNER)}/${encodeURIComponent(CONFIG.BACKEND_REPO)}/${encodeURIComponent(CONFIG.BRANCH || "main")}/${CONFIG.DATA_PATH}?v=${Date.now()}`;
  const res = await fetch(url, { cache: "no-store" });
  if (!res.ok) throw new Error(`Tidak bisa membaca db.json (${res.status})`);
...
async function saveDB(message = "Update data EAbsensi Satpam") {
  ...
  try {
    ...
    const api = `https://api.github.com/repos/${encodeURIComponent(CONFIG.GITHUB_OWNER)}/${encodeURIComponent(CONFIG.BACKEND_REPO)}/contents/${CONFIG.DATA_PATH}`;
    const getRes = await fetch(`${api}?ref=${encodeURIComponent(CONFIG.BRANCH || "main")}`, {
      headers: githubHeaders(token)
    });
    if (!getRes.ok) throw new Error(`Gagal ambil SHA data (${getRes.status})`);
    ...
    setSaveStatus("Berhasil tersimpan ke GitHub.");
    toast("Berhasil tersimpan ke backend GitHub.");
  } catch (err) {
    console.error(err);
    setSaveStatus("Gagal sinkron GitHub. Cek token / repo / permission.");
